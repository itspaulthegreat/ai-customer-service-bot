...
WIX_BASE_URL = os.getenv("WIX_BASE_URL", "https://kk185994.wixstudio.com/manly")
...
try:
    llm = ChatGroq(
        model_name="gemma2-9b-it",  # Updated to match PureAIAgent
        temperature=0.1,
        max_tokens=1200,
...
async def health_check():
    ...
    if use_ai_system:
        try:
            ...
            agent_healthy = bool(agent and hasattr(agent, 'llm') and hasattr(agent, 'wix_client'))
            return {
                "status": "healthy",
                "system": "pure_ai",
                "model": "gemma2-9b-it",
    ...
    try:
        ...
        if use_ai_system:
            ...
            return {
                ...
                "wix_url": wix_client.base_url,
                "new_arrivals_count": len(new_arrivals["data"]["products"]) if new_arrivals["success"] else 0,
                "sample_product": new_arrivals["data"]["products"][0] if new_arrivals["success"] and new_arrivals["data"]["products"] else "No products found",
                "test_status": "success" if connection_ok and new_arrivals["success"] else "failed",
